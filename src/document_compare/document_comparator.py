...
class DocumentComparatorLLM:
    def __init__(self):
        ...
        self.prompt=PROPMT_REGISTRY[PromptType.DOCUMENT_COMPARISON.value]
    ...
    def _format_response(self,response_parsed: list[dict])->pd.DataFrame:
        """
        Formats the response from LLM into a structured response.
        """
        try:
            df=pd.DataFrame(response_parsed)
            return df
        except Exception as e:
            self.log.error(f"Error in format_response", error=str(e))
