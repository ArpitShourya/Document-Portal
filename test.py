...
def test_doc_ingestion_and_rag():
    try:
        ...
        if not uploaded_files:
            print("No valid files path found")
            sys.exit(1)
        session_id="test_document_ingestion_and_rag"
        ingestor=DocumentIngestor(session_id=session_id)
        retriver=ingestor.ingest_file(uploaded_files)
        ...
        rag=ConversationalRAG(retriver=retriver,session_id=session_id)
        question="What is attention is all you need paper is about?"
        response=rag.invoke(question)
        print(f"\nQuestion: {question}\nAnswer: {response}") 
